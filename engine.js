/* Escape the Complex - game engine v1.1
   works with index.html, style.css and dynamic_rooms.json
   handles NPC behaviour & dialogue, gameplay logic & utility functions
   in-game items are contained in items.js
*/
...
function tellNpcAboutExit(npcName) {
  ...
  if (exits.teleporter && reactions.teleporter && (exitPreference === "teleport" || !flags.exitUnlocked)) {
    ...
    npcMoveToExit("scientist", "teleporter");
    return;
  ...
  if (exits.glass && reactions.glass && (exitPreference === "glass" || !flags.teleporterReady)) {
    ...
    npcMoveToExit("caretaker", "glass corridor");
    return;
...
// caretaker and scientist move towards favoured exits
function npcMoveToExit(npcId, exitId) {
  const npc = npcs[npcId];
  
  // arrays for NPC exit routes
  const exitRoutes = {
  ...
  if (npcId === "scientist" && exitId === "teleporter") {
    npc.target = "secret room";
    npc.route = exitRoutes.scientistToTeleporter;
    npc.routeIndex = 0;
  }

  if (npcId === "caretaker" && exitId === "glass corridor") {
    npc.target = "glass corridor";
    npc.route = exitRoutes.caretakerToGlass;
    npc.routeIndex = 0;
  }
}

// moves NPCs through the Complex
function updateNpcMovement() {
  Object.values(npcs).forEach(npc => {
    if (!npc.route.length) return;

    // already at destination
    if (npc.location === npc.target) return;
    
    // movement delay
    npc.moveCooldown = 2; // ticks between moves
    npc.currentCooldown = 0;

    if (npc.currentCooldown > 0) {
      npc.currentCooldown--;
      return;
    }
    ...
    if (npc.currentCooldown === 0) {
      npc.routeIndex++;
      npc.currentCooldown = npc.moveCooldown;
    }

    if (npc.routeIndex < npc.route.length) {
      npc.location = npc.route[npc.routeIndex];

      // arrival notification message if player is in the room
      if (npc.location === player.location) {
        appendMessage(`The ${npc.name} arrives.`);
      }
    }

    // reached destination
    if (npc.location === npc.target) {
      npc.route = [];
      npc.routeIndex = 0;
...
// triggers when caretaker/scientist arrive at exits
function onNpcArrive(npc) {
  if (npc.id === "scientist" && npc.location === "secret room") {
    appendMessage("The scientist rushes to the teleporter, frantically taking notes.");
  }

  if (npc.id === "caretaker" && npc.location === "glass corridor") {
    appendMessage("The caretaker gazes out through the glass, finally at peace.");
  }
}
...
function spawnVisitors() {
  ...
  if (player.location === "art gallery" || player.location === "fossil exhibit" || player.location === "observatory" || player.location === "gift shop") {
    appendMessage("You hear some echoing footsteps in the room, and turn to see a few new people wandering in. They must have come in through the glass corridor now that it's unlocked.");
  }
  ...
  giftShop.description.dynamic = "The once dusty and forgotten gift shop seems cleaner now. A couple of visitors wander between the shelves, looking at the trinkets and souvenirs. The automated check out has powered itself up, the screen showing a cheerful 'WELCOME!' to potential customers. ";
}
...
function playerState() {
  ...
  if (player.location === "white room" && inventory.includes("smallKey") && !flags.smallKeyholeRevealed && npcs.puppy.following) {
	...
	if (inventory.includes("smallKey")) {
	...
	} else if (inventory.includes("brassKey")) {
...
function advanceWorldState() {
  ...
  // customers should arrive after 15 turns, changed from 10 as it seemed too quick
  if (flags.customerTimer >= 15) {
    spawnCafeCustomers();
  }
  
  // visitors should start arriving a little before customers, changed from 7 to 10
  if (flags.visitorTimer >= 10) {
    spawnVisitors();
  }
  
  // moves the caretaker and scientist towards their favoured exits
  updateNpcMovement();
}
...
function goDirection(dir) {
  ...
  if (!room.exits || !room.exits[dir]) {
    appendMessage(`You can't go ${dir} from here. Look around to see where the exits are.`);
    return;
  }
...
const flavourCommands = (cmd) => {
  ...
  // pet the puppy
  else if (cmd.startsWith("pet") && (cmd.includes("puppy") || cmd.includes("digger"))) {
    petPuppy();
    handled = true;
  }
  // play with the puppy
  else if (cmd.startsWith("play") && (cmd.includes("puppy") || cmd.includes("digger"))) {
    playWithPuppy();
    handled = true;
  }
